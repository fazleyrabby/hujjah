...
### Embeddings
1. Transformers.js v3 `BAAI/bge-m3`: 1024 dims, multilingual (AR/BN/EN)
2. WebGPU much faster than WASM but not available on all devices
3. Generating embeddings in Python with sentence-transformers is faster than browser
4. SQLite stores embeddings as BLOB (binary) not strings
...
## 2026-04-26 — Narrator Bio Enrichment, Bengali Names, and Chain Explorer Maturity

### Narrator Enrichment State
- `narrators` in `src-tauri/resources/hujjah-hadith-core.db` now has full `tabaqah` coverage for all **24,184** narrators
- **528 narrators** have curated bio metadata (`name_en`, `birth_year`, `death_year`, `reliability`, `city`, `data_source='manual'`)
- The remaining **23,656** narrators are tagged `data_source='computed'` from chain-position analysis

### Bengali Narrator Name Pipeline
- `scripts/translate-bn.py` now works against a local MLX OpenAI-compatible endpoint
- All **528** curated narrators have `name_bn` populated
- The script was hardened to:
  - load `LLAMA_URL`, model id, timeout, and retry settings from env
  - strip model wrapper tokens such as `<|im_end|>`
  - cache by canonical `name_en` so Arabic case-ending variants reuse one Bengali form

### Chain Explorer Frontend
- `/chain` now has:
  - EN/BN/AR language-aware labels
  - richer narrator cards with `name_bn`, `name_en`, `city`, `reliability`, `tabaqah`, and `data_source`
  - `react-force-graph-2d` visualization via `components/NarratorGraph.tsx`
- `lib/chain-db.ts` now selects the extended narrator fields across search, hadith-chain lookup, and graph traversal

### Operational Findings
- The local VS Code `code-server` process may occupy or respawn on port `8080`, which collides with MLX if both are configured there
- `scripts/enrich-v2.py` required dynamic loading because the helper scripts use hyphenated filenames rather than import-safe module names
